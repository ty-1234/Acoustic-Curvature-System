...
from moveit_msgs.msg import CollisionObject, DisplayTrajectory, MotionPlanRequest, Constraints, PositionConstraint, \
    JointConstraint
from controller_manager_msgs.srv import ListControllers, ListControllersRequest, SwitchController, \
...
class FrankaDataCollection(object):
    ...
    def __init__(self):
        ...
        self.display_trajectory_publisher = rospy.Publisher('/move_group/display_planned_path',
                                                            ...
                                                            queue_size=20)  # Defines the topic on which the robot publishes the trajectory
        ...
        self.joint_home = [0.06841830223171314, -0.41813771533517224, -0.12852136230674158, -1.9645842290600706,
                           -0.03316074410412047, 1.6035182970937927, 0.8142089765552016]
        self.pose_home = ((0.4223942641978751, -0.04042834717301551, 0.6434531923890092),
                          (-0.9988081975410994, 0.039553416742988796, -0.027085673649872546, 0.009169407373883585))
        ...
        threading.Thread(target=self.pause_resume_listener, daemon=True).start()
    ...
    def get_ee_pose_tuple(self):
        ...
        rospy.loginfo(
            "End-Effector Position: x={}, y={}, z={}".format(self.current_pose.position.x, self.current_pose.position.y,
                                                             self.current_pose.position.z))
        rospy.loginfo("End-Effector Orientation: x={}, y={}, z={}, w={}".format(self.current_pose.orientation.x,
    ...
    def display_current_pose(self):
        ...
        rospy.loginfo(
            "End-Effector Position: x={}, y={}, z={}".format(pose.position.x, pose.position.y, pose.position.z))
        rospy.loginfo("End-Effector Orientation: x={}, y={}, z={}, w={}".format(pose.orientation.x, pose.orientation.y,
    ...
    def pause_resume_listener(self):
        """Thread for pause/resume functionality."""
        while True:
            input("Press Enter to pause/resume...")
            if self.pause_event.is_set():
                print("[INFO] Paused. Press Enter again to resume.")
    ...
    def wait_if_paused(self):
        """Pause execution if the pause event is cleared."""
    ...
    def go_to_next_loc(self, pose_goal):
        self.wait_if_paused()  # Pause/resume check
    ...
    def travel_AB(self):
        ...
        depth_list = [self.loc_A_depths, self.loc_B_depths, self.loc_C_depths, self.loc_D_depths]
        ...
        for i, loc in enumerate(locations, start=0):
            ...
            # -----------GOING to ALL DEPTHS of LOCATION {i} + GOING UP ------------------
            for j in range(n_depth):
                self.wait_if_paused()  # Pause/resume check

                start_time_depth = rospy.Time.now()
                success = self.go_to_next_loc(pose_goal=depth_list[i][j])  # Go to next depth
                if success:
                    rospy.loginfo(
                        f"DEPTH {j} achieved related to LOCATION {loc}:\n{depth_list[i][j]}.\nWaiting for {time_of_pushing} [s]")
                ...
                print(
                    f"Time spent for reaching DEPTH {j} in LOCATION {loc} (going down): {end_time_depth.to_sec()} [s]")

                # ------GOING UP ---------------
                self.wait_if_paused()  # Pause/resume check

                start_time_goup = rospy.Time.now()
                success = self.go_to_next_loc(pose_goal=loc_list[i])  # Go back to the main location
                if success:
                    rospy.loginfo(
                        f"LOCATION {loc} achieved for GOING UP: {loc_list[i]}.\nWaiting for {rebuild_time} [s]")
