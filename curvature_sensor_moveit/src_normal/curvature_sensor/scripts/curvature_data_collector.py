...
def main():
    ...
    # Construct the base filename and ensure uniqueness by appending a version number if needed.
    base_output_filename = f"raw_audio_curvature_{curvature_str_for_filename}"
    output_filename = f"{base_output_filename}.csv"
    counter = 1
    while os.path.exists(os.path.join(output_dir, output_filename)): # Check if file already exists
        output_filename = f"{base_output_filename}_v{counter}.csv" # Append version number
        counter += 1
