...
#
# This script records audio from a microphone, processes the audio in real time
# using FFT (Fast Fourier Transform) to extract frequency-domain features at
# specified target frequencies, and saves the extracted features along with
# curvature metadata to a CSV file.
#
# Intended Use:
#   - Collects labeled audio data for different radii of curvature (user input).
#   - Useful for experiments involving curvature sensors or acoustic analysis.
#
# Main Steps:
#   1. Checks for a working microphone and validates sample rate compatibility.
#   2. Prompts the user to enter the radius of curvature for the current test.
#   3. Continuously records audio, processes it in sliding windows, and extracts
#      FFT features for a range of frequencies.
#   4. Saves the processed data, including curvature and timestamps, to a CSV.
#
# Dependencies:
#   - numpy, pandas, sounddevice, logging, collections, datetime
#   - Local utility: curvature_fft_utils.py (for FFT feature extraction)
