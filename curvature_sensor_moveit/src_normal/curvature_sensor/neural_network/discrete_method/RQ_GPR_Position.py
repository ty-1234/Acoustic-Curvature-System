...
to predict 'Position_cm' based on FFT features.
...
1.  Data Loading and Preprocessing:
    ...
    -   For each file:
        ...
        -   Parses a 'group ID' (typically the position value) from the filename,
...
2.  Cross-Validation (Leave-One-Position-Out):
    -   If sufficient unique position groups exist (at least 2):
        -   Splits the data such that each fold uses one position group as the test set
...
TARGET_COLUMN = 'Position_cm'     # Changed from 'Curvature' to 'Position_cm'
ACTIVITY_COLUMN = 'Curvature_Active'
TIMESTAMP_COLUMN = 'Timestamp'
FEATURE_COLUMNS = FFT_COLUMNS  # Removed Position_cm from features since it's now the target

# --- Function: Parse Filename for Group (Position Value for a specific test object) ---
def parse_filename_for_group_position(filename, target_test_object_num_str):
    base_name = os.path.basename(filename)
    # Extract position from filename (modify this pattern based on your actual filename format)
    ...
    print(f"    Warning: Could not parse position group from filename: {base_name} for [test {target_test_object_num_str}] pattern. Using full filename stem as group.")
    return os.path.splitext(base_name)[0] 

# --- Main GPR Training and Evaluation Function ---
def train_gpr_for_selected_object(file_paths, target_test_object_num_str, output_model_dir_for_test_obj):
    ...
    for file_path in file_paths: 
        ...
        if not all(col in df_cleaned_input.columns for col in [TARGET_COLUMN, ACTIVITY_COLUMN] + FFT_COLUMNS):
        ...
        first_pos_zero_indices = df_cleaned_input[df_cleaned_input[TARGET_COLUMN] == 0.0].index
        ...
        # Normalize FFT Features of Active Data
        ...
        # Use position as the group ID for cross-validation
        group_id = parse_filename_for_group_position(base_filename, target_test_object_num_str)
    ...
    if len(unique_group_ids) < 2:
        ...
        print(f"\nStarting Leave-One-Position-Out Cross-Validation for test object '[test {target_test_object_num_str}]' using {len(unique_group_ids)} groups: {sorted(list(unique_group_ids))}...")
        ...
        for train_idx, test_idx in logo.split(X, y, groups):
            ...
            print(f"\n--- Fold {fold_num}: Testing on Position Group '{current_test_group}' (from [test {target_test_object_num_str}]) ---")
            ...
            fold_test_df = pd.DataFrame({
                'true_position': y_test.values, 'predicted_position': y_pred,
        ...
        if fold_metrics:
            ...
            print(f"\nCross-Validation Results Summary (Leave-One-Position-Out for [test {target_test_object_num_str}] data):")
            ...
            output_csv_path = os.path.join(output_model_dir_for_test_obj, f"gpr_test{target_test_object_num_str}_LOCO_position_cv_results.csv")
            try:
    ...
    if not X.empty and not y.empty:
        ...
        final_gpr_kernel_str = str(final_gpr.kernel_)
        ...
        try:
            ...
            model_save_path_str = model_save_path
            scaler_save_path_str = scaler_save_path
            generated_output_files.append(model_save_path)
            generated_output_files.append(scaler_save_path)
        except Exception as e:
...
# ... [keep the _generate_summary_report function exactly the same] ...
