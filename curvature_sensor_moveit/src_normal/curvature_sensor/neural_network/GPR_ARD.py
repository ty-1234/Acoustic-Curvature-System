"""
gpr_pipeline_ard_radius.py (Simplified for ARD Rational Quadratic Kernel, Radius Target)

A streamlined, well-commented script to perform an end-to-end machine learning pipeline
for predicting sensor radius using Gaussian Process Regression (GPR).
This version uses an ARD (Automatic Relevance Determination) Rational Quadratic kernel,
includes Position_cm as an input feature, predicts Radius_mm directly, and includes detailed error analysis.
It removes the flexibility of choosing different prediction targets to simplify debugging.
...
4.  Combine Data.
5.  Prepare Features (X) and Target (y - Radius_mm).
6.  Data Cleaning.
7.  Train-Test Split.
8.  Scaling (Features and Target).
9.  GPR Model Training (with ARD Rational Quadratic kernel).
10. Prediction.
11. Descaling.
12. Evaluation (MAE, RMSE, R2 on Radius_mm values).
13. Detailed Error Analysis (Segmented errors, Residual plots on Radius_mm).
14. Save outputs.
"""
...
from sklearn.gaussian_process.kernels import Kernel, RationalQuadratic, WhiteKernel, ConstantKernel as C
...
SESSIONS_TO_PROCESS = ["1"] # Modify as needed, e.g., ["1", "2"]
...
NUM_INPUT_FEATURES = len(INPUT_FEATURE_COLUMNS)

ACTIVITY_COLUMN = 'Curvature_Active'
CURVATURE_COLUMN = 'Curvature' # Original curvature (e.g., in mm^-1)
# Target for GPR and evaluation is now hardcoded to Radius_mm
...
# For ARD, length_scale will be an array of size NUM_INPUT_FEATURES.
# We initialize length_scale with a scalar; it will be expanded by the GPR.
# length_scale_bounds must match the number of dimensions if ARD is used.
length_scale_bounds_ard = [(1e-2, 1e2)] * NUM_INPUT_FEATURES # Ensure this is a list of tuples

GPR_KERNEL = C(1.0, (1e-3, 1e3)) * RationalQuadratic(
    length_scale=1.0,  # Initialize with a scalar for ARD
    alpha=1.0,
    length_scale_bounds=length_scale_bounds_ard, # Provide per-dimension bounds
    alpha_bounds=(1e-2, 1e2)
) + WhiteKernel(noise_level=0.1, noise_level_bounds=(1e-6, 1e1))

GPR_ALPHA = 1e-10 # Nugget term for numerical stability / observation noise
GPR_N_RESTARTS_OPTIMIZER = 10 # Number of optimizer restarts for kernel hyperparameters
...
if __name__ == "__main__":
    ...
    run_specific_dir_name = f"run_{run_timestamp}_ard_radius_sessions_{session_str_for_dir}" # Simplified name
    ...
    for file_path in files_to_process:
        ...
        # Calculate Radius_mm which will be the target
    ...
    # --- Stage 3: Prepare Features (X) and Target (y) & Clean ---
    if not all(col in combined_df.columns for col in INPUT_FEATURE_COLUMNS + [TARGET_RADIUS_COLUMN]):
        missing = [c for c in INPUT_FEATURE_COLUMNS + [TARGET_RADIUS_COLUMN] if c not in combined_df.columns]
        print(f"Error: Missing required columns in combined_df: {missing}. Exiting."); exit()

    X_combined = combined_df[INPUT_FEATURE_COLUMNS]
    y_combined = combined_df[TARGET_RADIUS_COLUMN] # Target is Radius_mm
    ...
    if VERBOSE and len(df_final_processed) < original_rows:
        print(f"  Dropped {original_rows - len(df_final_processed)} rows due to NaN/Inf values in features or target.")
    ...
    y = df_final_processed[TARGET_RADIUS_COLUMN] # Final target for GPR
    ...
    y_train_df = pd.DataFrame(y_train, columns=[TARGET_RADIUS_COLUMN]) # Target is Radius_mm
    ...
    print(f"  Using Kernel (initial parameters from get_params()):")
    try:
        initial_kernel_params = GPR_KERNEL.get_params(deep=True) 
        for param_name, param_value in initial_kernel_params.items():
            if param_name.endswith("length_scale") and isinstance(param_value, (list, np.ndarray)):
                param_value_list = list(param_value) if isinstance(param_value, np.ndarray) else param_value
                print(f"    {param_name}: array/list of {len(param_value_list)} items, e.g., {str(param_value_list[:2])}...{str(param_value_list[-2:]) if len(param_value_list) > 2 else str(param_value_list)}")
            elif isinstance(param_value, Kernel):
                 sub_kernel_params_str_list = []
                 for k, v_param in param_value.get_params(deep=False).items(): # Changed v to v_param
                     if not isinstance(v_param, Kernel) and not isinstance(v_param, (list, np.ndarray)):
                         sub_kernel_params_str_list.append(f"{k}={v_param}")
                 print(f"    {param_name}: {param_value.__class__.__name__}({', '.join(sub_kernel_params_str_list)})")
            else:
                print(f"    {param_name}: {param_value}")
    except Exception as e:
        print(f"    Could not fully print initial kernel parameters due to: {e}")
        print(f"    Raw GPR_KERNEL object: {GPR_KERNEL}")
    ...
    if VERBOSE:
        ...
        try:
            current_kernel_component = gpr.kernel_
            # Traverse common kernel structures to find the RationalQuadratic part
            if hasattr(current_kernel_component, 'k1') and hasattr(current_kernel_component.k1, 'k2') and \
               isinstance(current_kernel_component.k1.k2, RationalQuadratic):
                optimized_length_scales = current_kernel_component.k1.k2.length_scale
            elif hasattr(current_kernel_component, 'k2') and isinstance(current_kernel_component.k2, RationalQuadratic): # For C * RQ
                 optimized_length_scales = current_kernel_component.k2.length_scale
            elif isinstance(current_kernel_component, RationalQuadratic): # If RQ is the top or only main kernel part
                 optimized_length_scales = current_kernel_component.length_scale


            if optimized_length_scales is not None and isinstance(optimized_length_scales, (list, np.ndarray)):
                print(f"  Optimized ARD Length Scales ({len(optimized_length_scales)} values):")
                for feature, l_scale in zip(INPUT_FEATURE_COLUMNS, optimized_length_scales):
                    print(f"    {feature}: {l_scale:.4f}")
            else:
                print(f"  Could not directly access ARD length scales from expected kernel structure: {gpr.kernel_}")
        except Exception as e:
            print(f"  Error accessing ARD length scales: {e}")
        print(f"  Log-Marginal-Likelihood: {gpr.log_marginal_likelihood(gpr.kernel_.theta):.3f}")

    # --- Stage 7: Making Predictions ---
    if VERBOSE: print("\n--- Stage 7: Making Predictions ---") # Stage numbering was off
    y_pred_scaled, _ = gpr.predict(X_test_scaled, return_std=True)

    # --- Stage 8: Descaling Predicted Target ---
    ...
    # --- Stage 9: Set Evaluation Radius (y_pred_descaled is already radius) ---
    y_pred_evaluation_radius = y_pred_descaled 
    y_true_evaluation_radius = y_test # y_test is the true unscaled Radius_mm

    # --- Stage 10: Evaluating Model Performance ---
    ...
    base_filename_prefix = f"gpr_ard_rq_pos_radius_80_20" # Simplified as PREDICT_TARGET is fixed
    ...
    generate_true_vs_predicted_plot(
        ...
        target_name_for_plot="Radius", target_unit=radius_unit_eval
    )
    ...
    if max_true_rad > min_true_rad :
        ...
        if max_true_rad < RADIUS_CAP_VALUE * 0.9 :
            ...
            pre_cap_max = y_true_evaluation_radius[y_true_evaluation_radius < RADIUS_CAP_VALUE * 0.9].max()
            if pd.isna(pre_cap_max) or pre_cap_max <= min_true_rad : pre_cap_max = min_true_rad + ( (max_true_rad if max_true_rad < RADIUS_CAP_VALUE * 0.9 else 200) - min_true_rad) * (num_bins-1)/num_bins
    ...
    # --- JSON Output ---
    learned_ard_length_scales_dict = {}
    if 'optimized_length_scales' in locals() and optimized_length_scales is not None and isinstance(optimized_length_scales, (list, np.ndarray)):
        for feature, l_scale in zip(INPUT_FEATURE_COLUMNS, optimized_length_scales):
            learned_ard_length_scales_dict[feature] = l_scale

    output_summary = {
        "model_type": "GaussianProcessRegressor", "kernel_type": "ARD_RationalQuadratic",
        "target_predicted_by_gpr": TARGET_RADIUS_COLUMN, # Simplified
        "evaluation_always_on_radius": TARGET_RADIUS_COLUMN, # Simplified
        "input_features_used": INPUT_FEATURE_COLUMNS,
        "script_variant": f"80_20_ard_pos_target_radius", # Simplified
        ...
        "gpr_learned_ard_length_scales": learned_ard_length_scales_dict if learned_ard_length_scales_dict else "Not ARD or Not Available",
    ...
    example_table_df[f'True_{TARGET_RADIUS_COLUMN}'] = y_test_evaluation_radius_true
    ...
    print(f"  Example Results (first 5 test samples, GPR predicted RADIUS):") # Updated print
