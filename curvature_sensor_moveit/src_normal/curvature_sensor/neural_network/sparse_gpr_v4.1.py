"""
sparse_gpr_gpytorch_curvature_v4_1.py
...
This version fixes the noise initialization AttributeError from v4.

Key changes in v4.1:
- Corrected direct initialization of likelihood noise using the `likelihood.noise` property setter.
- Increased training iterations.
- Continued use of K-Means for inducing points and scikit-learn based kernel init.
...
TRAINING_ITERATIONS = 700 # Increased from 500
LEARNING_RATE = 0.01
...
SKLEARN_BEST_NOISE = 1e-5 # Target noise level for direct initialization

# Output directory
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_BASE_DIR = 'model_outputs'
RUN_ID = f"run_{TIMESTAMP}_sgpr_v4_1_rq_radius_pos_widefft_full" # Updated run ID
...
def save_metrics_and_config_to_json(output_summary, json_output_path):
    try:
        ...
        try:
            ...
            with open(json_output_path, 'w') as f: json.dump(output_summary, f, indent=4, sort_keys=True, default=fallback_default)
            if VERBOSE: print(f"\nMetrics and configuration saved to (with fallback): {json_output_path}")
        except Exception as e2: print(f"Fallback JSON saving also failed: {e2}")

def main():
    global NUM_INDUCING_POINTS

    if VERBOSE: print(f"--- SGPR Experiment Start (v4.1): {RUN_ID} ---") # Updated version in log
    if VERBOSE: print(f"Using GPU: {USE_GPU}" if USE_GPU else "Using CPU")

    # Stages 2-8 (File Discovery, Data Loading, Preprocessing, Split, Scaling) are identical to v3/v4
    ...
    current_num_inducing = NUM_INDUCING_POINTS
    if current_num_inducing >= num_train_samples:
        if VERBOSE: print(f"Warning: NUM_INDUCING_POINTS ({current_num_inducing}) >= num_train_samples ({num_train_samples}). Adjusting for K-Means.")
        current_num_inducing = num_train_samples - 1 if num_train_samples > 1 else 1
    
    if current_num_inducing < num_train_samples and current_num_inducing > 0 :
    ...
    if inducing_points.size(0) == 0 and num_train_samples > 0 :
         inducing_points = X_train_tensor[0:1, :].clone()
    ...
    likelihood = gpytorch.likelihoods.GaussianLikelihood(
        noise_constraint=gpytorch.constraints.GreaterThan(1e-7) # Ensure noise is positive and stable
    )
    initial_noise_tensor = torch.tensor([SKLEARN_BEST_NOISE]).float() # Target transformed noise value
    if USE_GPU: initial_noise_tensor = initial_noise_tensor.cuda()
    
    # Use the .noise property setter, which handles the inverse transform and initialization
    likelihood.noise = initial_noise_tensor 
    if VERBOSE: print(f"Likelihood noise directly initialized via property setter to target ~{SKLEARN_BEST_NOISE:.0e} (actual initial: {likelihood.noise.item():.7f})")
    ...
    output_summary['gpr_kernel_initial_config'] = {
        ...
        'direct_noise_initialization_target': SKLEARN_BEST_NOISE
    }
    ...
    for i in range(TRAINING_ITERATIONS):
        ...
        if VERBOSE and (i + 1) % 20 == 0:
            print(f'Iter {i+1}/{TRAINING_ITERATIONS} - Loss: {loss.item():.3f} ' +
                  ...
                  f'noise: {likelihood.noise.item():.7f}')
        optimizer.step()
    ...
    output_summary['log_marginal_likelihood_equivalent_ELBO'] = -loss.item()
    ...
    # --- Stage 14: Detailed Error Analysis & Plotting (Identical to v3) ---
    ...
    plt.title(f"SGPR (v4.1): True vs. Predicted (R²: {r2:.3f}, MAE: {mae:.2f} mm)"); plt.legend(); plt.grid(True) # Updated title
    ...
    plt.title("SGPR (v4.1): Residual Plot"); plt.grid(True) # Updated title
    ...
    radius_segments = [s for s in radius_segments if s[0] < s[1]]
    segmented_errors = []
    ...
    output_summary['model_type'] = "SparseGaussianProcessRegressor_GPyTorch_v4.1" # Updated model type
    ...
    if VERBOSE: print(f"\n--- SGPR Experiment End (v4.1): {RUN_ID} ---") # Updated version in log
