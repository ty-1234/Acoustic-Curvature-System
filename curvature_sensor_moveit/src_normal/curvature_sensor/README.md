# Curvature Sensing System
[TODO: License: MIT]

A comprehensive system for robotic curvature sensing through audio signal processing and machine learning.

## Table of Contents
- [Overview](#overview)
- [System Architecture](#system-architecture)
- [Requirements](#requirements)
- [Installation](#installation)
- [Workflow](#workflow)
- [Project Structure](#project-structure)
- [Scripts Description](#scripts-description)
- [Development and Extension](#development-and-extension)
- [Troubleshooting](#troubleshooting)
- [Contributing](#contributing)
- [License](#license)
- [Contact](#contact)

## Overview

The Curvature Sensing System is a modular framework that processes audio signals from robot-surface interactions to detect and analyze surface curvatures. It combines robotic control, audio processing, data collection, and visualization into a unified pipeline for tactile sensing research.

### Features

- **Audio Signal Generation**: Create test WAV files with controlled frequencies
- **Data Collection**: Record and extract FFT features from audio signals in real-time
- **ROS Integration**: Control robot movements and collect positional data
- **Data Processing**: Multi-stage pipeline for merging and preprocessing sensor data
- **Visualization**: Generate insightful plots and visualizations from FFT data
- **Modular Architecture**: Easy to extend with additional processing modules

## System Architecture

This system operates across two computers:

1. **Laptop**: Handles audio playback, recording, and FFT processing
2. **PC with ROS**: Controls the robot arm (Franka Emika) and manages physical interactions

The workflow involves generating test signals, playing/recording audio on the laptop, and simultaneously controlling the robot on the PC. Data from both systems is later merged using timestamps.

## Requirements

### Software Dependencies
- Python 3.6+
- ROS (Robot Operating System)
- Python packages (see `requirements.txt`)

### Hardware Requirements
- Compatible robot arm (tested with Franka Emika)
- Audio recording equipment
- Test surfaces with varying curvatures

## Installation

1. Clone the repository:
   ```bash
   git clone https://github.com/yourusername/curvature_sensor.git
   ```

2. Install the required packages:
   ```bash
   pip install -r requirements.txt
   ```

3. Set up ROS environment (on PC only):
   ```bash
   source /opt/ros/<ros_version>/setup.bash
   ```

## Workflow

The system is controlled through a menu-based interface in `main.py`:
```bash
python main.py
```

### Step 1: Generate Test Signal (Once Only)
- **Script**: `scripts/2000hz_wav_creator.py`
- **Run on**: Either machine
- **Output**: `300.0s.wav` (copy to laptop for playback)

### Step 2: Start Audio Playback Manually
- **Run on**: Laptop
- Manually play `300.0s.wav` using any audio player

### Step 3: Start Audio Recording & FFT Logging
- **Script**: `scripts/curvature_data_collector.py`
- **Run on**: Laptop
- **Output**: `csv_data/raw/raw_audio_<curvature>.csv`

### Step 4: Start Robot Movement and Clamp Pressing
- **Script**: `scripts/curvature_ros.py`
- **Run on**: PC (ROS machine)
- **Output**: `csv_data/raw/raw_robot_<curvature>.csv`

### Step 5: Wait Until Robot Finishes
- Stop the audio recording when robot completes its sequence

### Step 6: Merge CSVs by Timestamp
- **Script**: `scripts/csv_merger.py`
- **Run on**: Either machine
- **Output**: `csv_data/merged/merged_<curvature>.csv`

## Project Structure

```
curvature_sensor/
├── main.py                # Main entry point with menu interface
├── requirements.txt       # Python dependencies
├── 300.0s.wav            # Example WAV file
├── csv_data/             # Data storage directory
│   ├── merged/           # Combined audio and robot data
│   ├── preprocessed/     # Processed data ready for analysis
│   └── raw/              # Raw collected data
└── scripts/              # Processing modules
    ├── [2000hz_wav_creator.py](#2000hz_wav_creator)
    ├── [curvature_data_collector.py](#curvature_data_collector)
    ├── [curvature_ros.py](#curvature_ros)
    ├── [csv_merger.py](#csv_merger)
    ├── [curvature_data_processor.py](#curvature_data_processor)
    ├── [curvature_fft_utils.py](#curvature_fft_utils)
    └── [fft_visualizer.py](#fft_visualizer)
```

## Scripts Description

### 2000hz_wav_creator

**File:** [`scripts/2000hz_wav_creator.py`](scripts/2000hz_wav_creator.py)

Generates WAV files for testing with specific frequencies (200-2000 Hz at 200 Hz intervals).

**Usage:**
```bash
python scripts/2000hz_wav_creator.py
```

### curvature_data_collector

**File:** [`scripts/curvature_data_collector.py`](scripts/curvature_data_collector.py)

Records audio via microphone and performs real-time FFT analysis, saving amplitude features with timestamps.

**Usage:**
```bash
python scripts/curvature_data_collector.py <curvature_value>
```

### curvature_ros

**File:** [`scripts/curvature_ros.py`](scripts/curvature_ros.py)

Controls the robot arm using ROS to perform systematic movements and record positional data. Implements several controller types for the Franka Emika robot:

- **Cartesian Impedance Control**: Used for precise movement between locations
- **Force Control**: Manages pressing force during surface interaction
- **Error Recovery**: Handles recovery from force-related errors

**Usage:**
```bash
python scripts/curvature_ros.py <curvature_value>
```

### csv_merger

**File:** [`scripts/csv_merger.py`](scripts/csv_merger.py)

Combines audio FFT data and robot position data by matching timestamps.

**Usage:**
```bash
python scripts/csv_merger.py <curvature_value>
```

### curvature_data_processor

**File:** [`scripts/curvature_data_processor.py`](scripts/curvature_data_processor.py)

Processes merged data by normalizing values and preparing features for analysis.

**Usage:**
```bash
python scripts/curvature_data_processor.py <input_file> <output_file>
```

### curvature_fft_utils

**File:** [`scripts/curvature_fft_utils.py`](scripts/curvature_fft_utils.py)

Utility functions for FFT processing used by other scripts.

**Usage:**
```python
from scripts/curvature_fft_utils import calculate_fft, extract_frequency_amplitudes
```

### fft_visualizer

**File:** [`scripts/fft_visualizer.py`](scripts/fft_visualizer.py)

Creates visualizations from FFT data including spectrograms and amplitude plots.

**Usage:**
```bash
python scripts/fft_visualizer.py <data_file>
```

## Development and Extension

The system is designed to be modular and easily extendable:

1. Create a new script in the `scripts/` directory
2. Implement your functionality with a `main()` function
3. Add a new menu option in `main.py` that imports and calls your script

## Troubleshooting

### Audio Recording Problems
- **No audio detected**: Check microphone settings and connections
- **Low amplitude**: Adjust the gain on your audio interface
- **Distortion**: Reduce volume of audio playback or microphone sensitivity

### ROS Communication Issues
- **Connection refused**: Ensure ROS_MASTER_URI is set correctly on both machines
  ```bash
  # On the ROS PC (master)
  export ROS_MASTER_URI=http://localhost:11311
  
  # On the laptop (client)
  export ROS_MASTER_URI=http://<ros-pc-ip>:11311
  export ROS_IP=<laptop-ip>
  ```

## Contributing

1. Fork the repository
2. Create a feature branch (`git checkout -b feature/amazing-feature`)
3. Commit your changes (`git commit -m 'Add amazing feature'`)
4. Push to the branch (`git push origin feature/amazing-feature`)
5. Open a Pull Request

## License

[TODO: Add MIT License details]

## Contact

[TODO: Add contact information]



